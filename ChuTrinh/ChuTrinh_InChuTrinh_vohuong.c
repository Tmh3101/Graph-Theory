...
void delete_element(List *list, int p){
	int i;
...
void DFS_Recursion(Graph *G, int u){
	...
	for(i = 1; i <= L.size; i++){
		...
		if(color[v] == WHITE) {
			parent[v] = u;
			DFS_Recursion(G, v); //neu v chua duyet -> duyet no
		}
		else if(color[v] == GRAY && parent[u] != v){
			push_back(&bplt, v);
			has_circle = 1; //neu v dang duoc duyet->quay lai mot nut nao do-> co chua chu trinh
			while(element_at(&bplt, 1) != v){
				delete_element(&bplt, 1);
			}
		}
	}
	//danh dau u da duyet
...
int main(){
	...
	if(has_circle) {
		for(i = 1; i <= bplt.size; i++){
			printf("%d ", element_at(&bplt, i));
		}
	}
	else printf("-1");
